...
export class GuardSpineImageGuard implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'GuardSpine ImageGuard',
    name: 'guardSpineImageGuard',
    group: ['transform'],
    version: 1,
    subtitle: 'Image | {{$parameter["policyPackId"]}} | Block >= L{{$parameter["riskThreshold"]}}',
    description: 'Send image artifact content to GuardSpine for policy review',
    defaults: { name: 'ImageGuard' },
    inputs: ['main'],
    outputs: ['main', 'main'],
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        throw new NodeOperationError(
          this.getNode(),
          `ImageGuard API error: ${error.message}`,
          { itemIndex: i },
        );
      }
